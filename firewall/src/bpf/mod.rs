...
use firewall_common::{CONFIG_INDEX_FLAGS, CONFIG_INDEX_ICMP, CONFIG_INDEX_MODE, CONFIG_INDEX_RATE_PPS};
...
fn set_config(
    ...
    config_map
        .set(CONFIG_INDEX_RATE_PPS, rate_pps, 0)
        .context("failed to set rate PPS in CONFIG map")?;
    Ok(())
}
